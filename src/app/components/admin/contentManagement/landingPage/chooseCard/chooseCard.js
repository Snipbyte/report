...
const ChooseCard = () => {
  ...
  useEffect(() => {
    const fetchSection = async () => {
      try {
        const data = await getSections("choose");
  ...
  const handleLanguageSwitch = (lang) => {
    setDescription((prev) => ({ ...prev, [language]: currentEditorContent })); // Save current editor content
    setLanguage(lang);
    setCurrentEditorContent(description[lang] || ""); // Set editor content for new language
  };
  ...
  const onSubmit = async () => {
    ...
    if (!heading.English || !heading.French) {
      alert("Headings in both languages are required.");
      return;
    }

    if (!description.English || !description.French) {
