...
const EditBlog = ({ blogData, onSuccess }) => {
  const [message, setMessage] = useState("");
  const [isError, setIsError] = useState(false);
  const [imgurl, setImgurl] = useState(blogData.thumbnailImage || null);
  ...
  const onSubmitBlog = async (data) => {
    try {
      const response = await axios.post("/api/blogs/edit", {
        slug: blogData.slug, // Pass the slug to identify the blog
        ...data,
        tags,
